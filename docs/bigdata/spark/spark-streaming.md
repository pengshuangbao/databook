...
![image](https://static.lovedata.net/jpg/2018/6/14/4c10e84fe55db71e105e0a2a7c3ad10a.jpg)
![image](https://static.lovedata.net/jpg/2018/6/14/4cd553817fa7f91d206508dad84ded7a.jpg)
![image](https://static.lovedata.net/jpg/2018/6/14/854a01530c476d05f6987ac329873022.jpg)
![image](https://static.lovedata.net/jpg/2018/6/14/8c6963c0dfb3f36398c7b43a823889d6.jpg)
...
- 模块 1：DAG 静态定义
  ...
  - DStream 和 RDD 的关系
    ...
    - ![image](https://static.lovedata.net/jpg/2018/6/14/a704df7e8a3741d155cf29be66954f81.jpg)
    - ![Apache Storm  和 storm 不同的地方](https://static.lovedata.net/jpg/2018/6/14/4084d7de41feac45d85e112841f3b5a5.jpg)
- 模块 2：Job 动态生成
  ...
  - ![image](https://static.lovedata.net/jpg/2018/6/14/c9db933b1dfacee3182ebfe4321ab02a.jpg)
  - JobGenerator 维护了一个定时器，周期就是我们刚刚提到的 batchDuration，定时为每个 batch 生成 RDD DAG 的实例。
    ...
    - ![image](https://static.lovedata.net/jpg/2018/6/14/db592ed6c11c4075d92d4ceb99866b5b.jpg)
- 模块 3：数据产生与导入
  - ![image](https://static.lovedata.net/jpg/2018/6/14/1c7d68c6923522fa5c46adf289221e2f.jpg)
- 模块 4：长时容错 数据安全
  - executor
    ...
    - (3) 重放： kafka 重放
    - (4) 忽略
    - ![image](https://static.lovedata.net/jpg/2018/6/14/31e647f87df8e5e9cf4790a53c2c8e23.jpg)
  - driver 端长时容错
  ...
  - ![image](https://static.lovedata.net/jpg/2018/6/14/8db48d0ec7659a00e04dd6410f278cfc.jpg)

![image](https://static.lovedata.net/jpg/2018/6/14/d1d50e147a96b818046405f877daaddd.jpg)
...
3. ![image](https://static.lovedata.net/jpg/2018/6/14/8f248fec79f96bd8aa5eb7b04214c5bb.jpg)
...
7. ![image](https://static.lovedata.net/jpg/2018/6/14/379be35f9037857f5531cf7d10fa486d.jpg)
8. 总结一下：
...
9. ![image](https://static.lovedata.net/jpg/2018/6/14/18341bbd7cc3323b903a114d01d3c524.jpg)
10. Spark Streaming 在进行物理记录时却是反向 相同与RDD 的先计算上游的思路   DStream 也采用的反向表示 所以，这里 d 对 c 的引用，表达的是一个上游依赖（dependency）的关系；也就是说，不求值则已，一旦 d.print() 这个 output 操作触发了对 d 的求值，那么就需要从 d 开始往上游进行追溯计算。
    1. ![image](https://static.lovedata.net/jpg/2018/6/14/69b51ce16fdb7e073b0314474202bb8b.jpg)
11. 我们总结一下：
...
12. ![image](https://static.lovedata.net/jpg/2018/6/14/47a52951ef1abddc807788b5a6710b8d.jpg)
...
1. Spark Streaming 的 模块 1 DAG 静态定义 要解决的问题就是如何把计算逻辑描述为一个 RDD DAG 的“模板”，在后面 Job 动态生成的时候，针对每个 batch，都将根据这个“模板”生成一个 RDD DAG 的实例。![image](https://static.lovedata.net/jpg/2018/6/14/854a01530c476d05f6987ac329873022.jpg)
...
3. DStream 通过 generatedRDD 管理已生成的 RDD 
    ...
    2. ![image](https://static.lovedata.net/jpg/2018/6/14/356a6d239b4011a2c2612a587c1e3560.jpg)
    3. 每一个不同的 DStream 实例，都有一个自己的 generatedRDD ![image](https://static.lovedata.net/jpg/2018/6/14/a704df7e8a3741d155cf29be66954f81.jpg)
4. 实现
...
2. Spark Streaming 的 Job 总调度者 JobScheduler
    ...
    4. ![image](https://static.lovedata.net/jpg/2018/6/14/db592ed6c11c4075d92d4ceb99866b5b.jpg)
    5. 就将其包装成一个 JobSet（如上图 (3) ），然后就调用 JobScheduler.submitJobSet(jobSet) 来交付回 JobScheduler
    6. ![image](https://static.lovedata.net/jpg/2018/6/14/b8b45b6774686732cef2483d6ae3d86e.jpg)
    7. ![image](https://static.lovedata.net/jpg/2018/6/14/2bddb82870c5fe3f8799fe0828bd9bdc.jpg)
    8. JobHandler 除了做一些状态记录外，最主要的就是调用 job.run()
3. Spark Streaming 的 JobSet, Job，与 Spark Core 的 Job, Stage, TaskSet, Task
    1. ![image](https://static.lovedata.net/jpg/2018/6/14/57491e55dcc5992b0b544fecdbd44292.jpg)
...
1. ![image](https://static.lovedata.net/jpg/2018/6/14/d6c70b578147b38650a148e27262d1d6.jpg)
2. 在启动了 RPC 处理线程 eventLoop 后，就会根据是否是第一次启动，也就是是否存在 checkpoint，来具体的决定是 restart() 还是 startFirstTime()。
3. ![image](https://static.lovedata.net/jpg/2018/6/14/049cb705cae472db4447bab4f5b7a340.jpg)
...
5. RecurringTimer
    ...
    2. ![image](https://static.lovedata.net/jpg/2018/6/14/ed1a17fad3d8c12b9f3c3d48ec0c8dd8.jpg)
    3. 整个 timer 的调度周期就是 batchDuration，每次调度起来就是做一个非常简单的工作：往 eventLoop 里发送一个消息 —— 该为当前 batch (new Time(longTime)) GenerateJobs 了！
6. GenerateJobs
    1. ![image](https://static.lovedata.net/jpg/2018/6/14/d520aabf0e670b3a357a85f995a992cc.jpg)
...
7. 整个 DStreamGraph 是由 output stream 通过 dependency 引用关系，索引到上游 DStream 节点。而递归的追溯到最上游的 InputDStream 节点时，就没有对其它 DStream 节点的依赖了，因为 InputDStream 节点本身就代表了最原始的数据集。
 ![image](https://static.lovedata.net/jpg/2018/6/14/47a52951ef1abddc807788b5a6710b8d.jpg)
...
1. ReceiverTracker 自身运行在 driver 端，是一个管理分布在各个 executor 上的 Receiver 的总指挥者。在 ssc.start() 时，将隐含地调用 ReceiverTracker.start()；而 ReceiverTracker.start() 最重要的任务就是调用自己的 launchReceivers() 方法将 Receiver 分发到多个 executor 上去。然后在每个 executor 上，由 ReceiverSupervisor 来分别启动一个 Receiver 接收数据
    ![image](https://static.lovedata.net/jpg/2018/6/14/224b9ad7f4aa02d535ecc1a7930b84f6.jpg)
2. 通过 1 个 RDD 实例包含 x 个 Receiver，对应启动 1 个 Job 包含 x 个 Task，就可以完成 Receiver 的分发和部署了。上述 (1.a)(1.b)(1.c)(2) 的过程示意如下图：
    1. ![image](https://static.lovedata.net/jpg/2018/6/14/cb904de991c8e5292080411794978562.jpg)
...
1. ![image](https://static.lovedata.net/jpg/2018/6/14/2a3cb6bdc7b04cb41c90c04a9b1e7d08.jpg)
...
1. ![image](https://static.lovedata.net/jpg/2018/6/14/f922757b3191a274fd1d7a0e6a9e6868.jpg)
2. ![image](https://static.lovedata.net/jpg/2018/6/14/98d3fc9cb3c2a8acf556428ac2a8ace2.jpg)

###  常用名词介绍

1. Dstream
    1. ![image](https://static.lovedata.net/jpg/2018/6/15/2b403056ff561e785fda3803acb7a8d0.jpg)
    2. ![image](https://static.lovedata.net/jpg/2018/6/15/f4986920bb874a1e97681025b19f8133.jpg)
    3. ![image](https://static.lovedata.net/jpg/2018/6/15/3d0a26d27d1edbc997f753f04640ac27.jpg)
    4. ![image](https://static.lovedata.net/jpg/2018/6/15/3136b9d092d4fbaaba9ded7fffde1943.jpg)

###  SS与Storm的对比

![image](https://static.lovedata.net/jpg/2018/6/19/d25e02bd208bec1aab06ccaeb47027f4.jpg)
![image](https://static.lovedata.net/jpg/2018/6/19/edfed7795fb19be856de49e138064267.jpg)
![image](https://static.lovedata.net/jpg/2018/6/19/179518a440011f913564852609cbf3e0.jpg)
...
尽量加上友好的错误提示信息。开放接口层要将异常处理成错误码和错误信息方式返回。Java 开发手册
43/57
3. 【参考】 分层领域模型规约：
...
举类型或者包含枚举类型的 POJO 对象。Java 开发手册
44/57
...
主流的linux服务器默认所支持最大fd数量为1024，当并发连接数很大时很容易因为 fd不足而出现“openJava 开发手册
45/57
...
但是大家在设计评审中经常出现语焉不详，甚至是词不达意的情况。准确地表达系统难点需要具备如下能Java 开发手册
48/57
...
⚫ 自定义的控件类型需明确交互方式。
正例： 用户登陆场景中，输入框的按钮都需要考虑 tab 键聚焦，符合自然逻辑的操作顺序如下，“输入用
...
1.4.0 详尽版 2018.05.20 增加设计规约大类，共 16 条。Java 开发手册
50/57
版本号 版本名 发布日期 备注
...
4） 完善若干处示例。比如， ISNULL 的示例等Java 开发手册
51/57
附 2： 专有名词解释
...
13.三方库: 公司之外的开源库（ jar 包）。Java 开发手册
52/57
附 3： 错误码列表
错误码 中文描述 说明
